...
@tree.command(
  name="avatar",
  description="Sends the avatar of a specified member to the channel.")
async def avatar(interaction: discord.Interaction,
                 member: discord.Member,
                 use_server_avatar: str = "default"):
  # Send the member's avatar to the channel
  if use_server_avatar.lower() == "server" or use_server_avatar.lower(
  ) == "true" or use_server_avatar.lower() == "yes":
...
@tree.command(name="spam", description="Sends a spam message.")
...
async def addcommand(interaction: discord.Interaction, message: str,
  ...
  if interaction.channel.permissions_for(interaction.user).manage_messages:
    ...
    if interaction.channel.permissions_for(
        interaction.guild.me).manage_messages:
      ...
      await interaction.response.send_message(
        f"Command '{message}' added successfully!")
  else:
    await interaction.response.send_message(
      "You do not have the necessary permissions to add commands.")
    ...
    if interaction.channel.permissions_for(
      ...
      if message in commands:
        ...
        with open("commands.txt", "w") as f:
          f.write(commands_json)
        await interaction.response.send_message(
          f"Command '{message}' removed successfully!")
      else:
        await interaction.response.send_message(
          f"Command '{message}' does not exist.")
  else:
    await interaction.response.send_message(
      "You do not have the necessary permissions to remove commands.")
...
async def listcommands(interaction: discord.Interaction):
  command_list = "\n".join(
    [f"{message}: {answer}" for message, answer in commands.items()])
  output = f"Commands:\n```\n{command_list}\n```"
...
async def on_message(interaction: discord.Interaction):
  ...
  interaction.content = interaction.content.lower()
